...
import com.harana.datagrid.DatagridLocationClass;
...
public class DatanodeStorage {
	...
	private final DatagridStorageClass storageClass;
	private final DatagridLocationClass locationClass;
	private final NamenodeConnection namenodeConnection;

	public DatanodeStorage(int storageType, DatagridStorageClass storageClass, InetSocketAddress serverAddress, NamenodeConnection namenodeConnection) throws Exception {
	...
	public void setBlock(long lba, long addr, int length, int key) throws Exception {
		...
		NamenodeVoid res = this.namenodeConnection.setBlock(blockInfo).get(DatagridConstants.RPC_TIMEOUT, TimeUnit.MILLISECONDS);
	...
	public DatanodeStatistics getDataNode() throws Exception {
		InetSocketAddress inetAddress = serverAddress;
		DatanodeInfo dnInfo = new DatanodeInfo(storageType, storageClass.value(), locationClass.value(), inetAddress.getAddress().getAddress(), inetAddress.getPort());
		return this.namenodeConnection.getDataNode(dnInfo).get(DatagridConstants.RPC_TIMEOUT, TimeUnit.MILLISECONDS).getStatistics();
	}	
}